...
TextBox::TextBox(const std::string& text) {
  ...
  set_on_key_down([this, copy_text_to_clipboard, read_clipboard_text](int32_t key) {
    ...
    if (!shift) {
      // Any other keypress clears selection.
      clear_selection();
    }
  });
}

void TextBox::draw(graphics::Renderer& renderer) {
  renderer.draw_rect(x_, y_, width_, height_, style_.background, style_.corner_radius);

  auto border = style_.border;
  if (focused_) {
    border = style_.foreground;
  }
  ...
  renderer.draw_text(text_x, text_y, text_, style_.foreground, style_.font_family, style_.font_size);
